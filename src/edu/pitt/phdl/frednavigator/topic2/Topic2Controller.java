...
public class Topic2Controller implements Initializable {
  ...
  String r0Value = "1.6";
  String baselineR0Value = "1.6";
  String immunizationValue = "0.0";
  String baselineImmunizationValue = "0.0";
  String fileVariable;
  ...
  private Tab tabInfo;
  @FXML
  private BorderPane brdrPaneInfo;
  @FXML
  private Tab tabSEIR;
  @FXML
 ...
 //Prediction Tab controls
  ...
  private void startPage(ActionEvent event)
  {
    try
    {
      ...
      homepageLayout = FXMLLoader.load(getClass().getResource(FredNavigatorConstants.HOMEPAGE_FXML));
  ...
  private void setBaselineValues()
  {
    ...
    this.baselineImmunizationValue = this.immunizationValue;
    
    ObservableList<Tab> tabs = this.tabPaneTopic2.getTabs();
  ...
  private void navigatePredictionsTab()
  {
    ...
    if(this.btnPredictionNext.getText().equals("Begin"))
    {
      ...
      this.predictionImgVw.setImage(new Image(this.getClass().getResourceAsStream("/edu/pitt/phdl/"
              ...
              + FredNavigator.topicTwoQuestions[this.currentQuestion].getImageFileName())));
      this.questionTxt.setText(FredNavigator.topicTwoQuestions[this.currentQuestion].getQuestionText());
      this.answerTxt.setText(FredNavigator.topicTwoQuestions[this.currentQuestion].getAnswerText().trim().replaceAll(" +", " "));
      this.currentQuestion++;
    }
    else if(this.currentQuestion < FredNavigator.topicTwoQuestions.length)
    {
      this.predictionImgVw.setImage(new Image(this.getClass().getResourceAsStream("/edu/pitt/phdl/"
              + "frednavigator/resource/image/" 
              + FredNavigator.topicTwoQuestions[this.currentQuestion].getImageFileName())));
      ...
      if(this.currentQuestion == FredNavigator.topicTwoQuestions.length - 1)
      {
  ...
  private void makeLineChartOnSelectedTab(Tab selectedTab)
  {
    ...
    if(selectedTab == this.tabSEIR)
    {
      ...
      lineChart.setTitle("SEIR: R0 = " + this.r0Value 
              + " / Immunization Rate = " 
              + Double.parseDouble(this.immunizationValue) * 100 + "%");
      lineChart.setPrefWidth(1000);
      lineChart.setPadding(new Insets(0.0, 100.0, 0.0, 0.0));
      ...
      setPopupOnHover(seriesI, "Infectious: R0 = " + this.r0Value 
              + " / Immunization Rate = " 
              + Double.parseDouble(this.immunizationValue) * 100 + "%");
      setPopupOnHover(seriesR, "Recovered: R0 = " 
              + this.r0Value + " / Immunization Rate = " 
              + Double.parseDouble(this.immunizationValue) * 100 + "%");
    ...
    else if(selectedTab == this.tabAttackRate) 
    {
      ...
      lineChart.setTitle("Attack Rate: R0 = " + this.r0Value + 
              " / Immunization Rate = " + Double.parseDouble(this.immunizationValue) * 100 + "%");
      lineChart.setPrefWidth(1000);
      HBox hBox = new HBox();
      hBox.getChildren().add(lineChart);
      ...
      XYChart.Series baseAR = makeLineChartSeries("AR", "Attack Rate Baseline", 
              this.baselineR0Value, this.baselineImmunizationValue);
      ...
      setPopupOnHover(baseAR, "Attack Rate Baseline: R0 = " + this.baselineR0Value 
              + " / Immunization Rate = "
              + Double.parseDouble(this.baselineImmunizationValue) * 100 + "%");
    ...
    else if(selectedTab == this.tabPrevalence) 
    {
      ...
      setPopupOnHover(seriesP, "Prevalence: R0 = " + this.r0Value 
              + " / Immunization Rate = " 
              + Double.parseDouble(this.immunizationValue) * 100 + "%");
      setPopupOnHover(baseP, "Clinical Attack Rate Baseline: R0 = " + this.baselineR0Value
              + " / Immunization Rate = " 
              + Double.parseDouble(this.baselineImmunizationValue) * 100 + "%");
    } 
  ...
  {
    return makeLineChartSeries(variable, seriesName, this.r0Value, this.immunizationValue);
  }
   ...
   * @return 
   */
  private XYChart.Series makeLineChartSeries(String variable, String seriesName, String chosenR0Value, String chosenImmunizationValue)
  {
    //defining a series
    ...
    String dataFile =
            ...
            + FredNavigator.keyToJob.get("R0=" + chosenR0Value  
                      + "-Immunization=" + chosenImmunizationValue)
            + FredNavigatorConstants.FS + "DATA" + FredNavigatorConstants.FS 
  ...
  /**
   * This is run when Topic 2 is opened
   * 
   * @param url
   * @param rb
   */
    ...
    this.tabPaneTopic2.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Tab>() {
      ...
      public void changed(ObservableValue<? extends Tab> tab, Tab oldTab, Tab newTab) {
        ...
        if(newTab == tabInfo || newTab == tabPrediction) 
        {
          ...
          immunizationSlider.setDisable(true);
          btnPlayMovie.setDisable(true);   
          lblR0.setDisable(true);
          lblPctImmune.setDisable(true);
        }
        else
        {
          r0Slider.setDisable(false);
          immunizationSlider.setDisable(false);
          btnPlayMovie.setDisable(false);
          lblR0.setDisable(false);
          lblPctImmune.setDisable(false);
        }
